from home.image_module import bytes_manipulation as bm
from home.video_module import video
from os import path
from home.main_pack import message
from home.main_pack import utils
import sys
import cv2
...
def retrieve(video_file_location, key_file):
    ...
    video_file.release()
    print(final_name, extracted_bytes_array)
    # create the hidden file
    result = message.write_file(final_name, extracted_bytes_array)

    if result:
        print('Message extracted successful:',extracted_bytes_array)
    else:
